def call(String auth_url)
{
    ...
    sh (returnStdout: false, script: "/bin/bash ${WORKSPACE}/hack/capture-artifacts.sh || true")
    KEYPAIR=sh(returnStdout: true, script: "cd ${WORKSPACE}/deploy && make terraform:output TERRAFORM_DIR=.${TARGET} TERRAFORM_OUTPUT_VAR=keypair-name || true") 
