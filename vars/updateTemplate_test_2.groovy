def call(String authurl, String pvczone, String distroimage, String mast, String work, String manger, String vul, String prox, String gluster, String disto)
{
    ...
    {
        sh " cd ${WORKSPACE}/canary-deployments/templates &&  sed -i \"\$(( \$( wc -l < \"${TERMPLATE_FILE}\")))s|\$|\\n\"icp_configuration\" \"{\"|\" \"${TERMPLATE_FILE}\""
        sh " cd ${WORKSPACE}/canary-deployments/templates &&  grep -q 'icp_configuration' \"${TERMPLATE_FILE}\" && sed -i  -e '/icp_configuration/r ${WORKSPACE}/config.tf' \"${TERMPLATE_FILE}\""
        sh " cd ${WORKSPACE}/canary-deployments/templates &&  echo \"}\" >> \"${TERMPLATE_FILE}\""
        sh " cd ${WORKSPACE}/canary-deployments/templates &&  cat \"${TERMPLATE_FILE}\" "
        if (env.DEPLOY_VA == "false")
        {
            ...
            sh " cd ${WORKSPACE}/canary-deployments/templates && grep -q ' *vulnerability-advisor.*=.*enabled' \"${TERMPLATE_FILE}\" && sed -i \"s|.*vulnerability-advisor.*=.*\$|\\\"vulnerability-advisor\\\" = \\\"disabled\\\"|g\" \"${TERMPLATE_FILE}\" || true"    
        }
        else if (env.DEPLOY_VA == "true")
        {
            ...
            sh " cd ${WORKSPACE}/canary-deployments/templates && grep -q ' *vulnerability-advisor.*=.*disabled' \"${TERMPLATE_FILE}\" && sed -i \"s|.*vulnerability-advisor.*=.*\$|\\\"vulnerability-advisor\\\" = \\\"enabled\\\"|g\" \"${TERMPLATE_FILE}\" || true"
        }
        ...
        if (env.ENABLE_GLUSTER == "true")
        {
            ...
            sh " cd ${WORKSPACE}/canary-deployments/templates && grep -q ' *storage-glusterfs.*=.*disabled' \"${TERMPLATE_FILE}\" && sed -i \"s|.*storage-glusterfs.*=.*\$|\\\"storage-glusterfs\\\" = \\\"enabled\\\"|g\" \"${TERMPLATE_FILE}\"|| true "
        }
        else if (env.ENABLE_GLUSTER == "false")
        {
            ...
            sh " cd ${WORKSPACE}/canary-deployments/templates && grep -q 'storage-glusterfs.*=.*enabled' \"${TERMPLATE_FILE}\" && sed -i \"s|.*storage-glusterfs.*=.*\$|\\\"storage-glusterfs\\\" = \\\"disabled\\\"|g\" \"${TERMPLATE_FILE}\"|| true "
        }
    ...
    if (env.ENABLE_MCM == "true")
    {
        sh " cd ${WORKSPACE}/canary-deployments/templates && grep -q 'multicluster-endpoint.*=' \"${TERMPLATE_FILE}\" && sed -i \"s|.*multicluster-endpoint.*=.*\$|\\\"multicluster-endpoint\\\" = \\\"enabled\\\"|g\" \"${TERMPLATE_FILE}\" || sed -i '/management_services.*=.*{/a  multicluster-endpoint = \\\"enabled\\\"' \"${TERMPLATE_FILE}\""
    }
    if (env.ENABLE_NPD == "true")
    {
        sh " cd ${WORKSPACE}/canary-deployments/templates && grep -q 'node-problem-detector-draino.*=' \"${TERMPLATE_FILE}\" && sed -i \"s|.*node-problem-detector-draino.*=.*\$|\\\"node-problem-detector-draino\\\" = \\\"enabled\\\"|g\" \"${TERMPLATE_FILE}\" || sed -i '/management_services.*=.*{/a  node-problem-detector-draino = \\\"enabled\\\"' \"${TERMPLATE_FILE}\""
    }
    if (env.ENABLE_PSN == "true")
    {
        sh " cd ${WORKSPACE}/canary-deployments/templates && grep -q 'platform-security-netpols.*=' \"${TERMPLATE_FILE}\" && sed -i \"s|.*platform-security-netpols.*=.*\$|\\\"platform-security-netpols\\\" = \\\"enabled\\\"|g\" \"${TERMPLATE_FILE}\" || sed -i '/management_services.*=.*{/a  platform-security-netpols = \\\"enabled\\\"' \"${TERMPLATE_FILE}\""
    }
    if (env.ENABLE_MINIO == "true")
    {
        sh " cd ${WORKSPACE}/canary-deployments/templates && grep -q 'storage-minio.*=' \"${TERMPLATE_FILE}\" && sed -i \"s|.*storage-minio.*=.*\$|\\\"storage-minio\\\" = \\\"enabled\\\"|g\" \"${TERMPLATE_FILE}\" || sed -i '/management_services.*=.*{/a  storage-minio = \\\"enabled\\\"' \"${TERMPLATE_FILE}\""
    }
}
